...
L=4 # number of circuit layer to be aplied
...
print('Divergence with increasing number of layer is due to non-perfect optimization. I know my QC is ok, but the optimization method and parameters should be chose carefull. I could not have time to do this. But simple grid search can be done')
...
print('Since double qubit gate circuit is diagonal in computational basis, to create identity operation one needs RZ gate which is diagonal also in the computational basis. Therefore as long as one of the layers include RZ gate, the circuit can be identity with optimized angles')
